...
# Add some dummy vars
means_to_plot$shape_F = 'Observed Mean'
random_to_plot2$shape_F = 'Random Mean'

string_perm_res_dots = ggplot(random_to_plot2,aes(x = random_mean,y = climate_var_F)) +
  geom_point(data = means_to_plot,aes(y = climate_var_F,x = boot_mean,shape = shape_F),
             colour = "forestgreen",
             size = 7) +
  ...
  geom_point(aes(colour=N,shape = shape_F), size=6) + 
  ...
  scale_shape_manual(values = c('Random Mean' = 16,
                                'Observed Mean' = 17)) +
  xlim(0,1.25*(max(means_to_plot$boot_mean))) +
  labs(x = "Within-network interactions",colour = "Orthogroup N",shape = '') +
  guides(colour = guide_colourbar(title.position="top", title.hjust = 0.5),
         shape = guide_legend(override.aes = list(colour = 'black',fill = 'black')))
...
go_bars_fig = ggplot(go_bars_data,aes(y = go_name_F,x = -log10(fdr))) +
  ...
  scale_x_continuous(breaks = c(-log10(0.5),
                                ...
                                -log10(0.05),
                                -log10(0.01)),
                     labels = as.character(c(0.5,'','',0.2,0.1,0.05,0.01)))
go_bars_fig
...
# dev.off()
pdf("figs/Figure4_network_interactions_and_GO.pdf",width = 12,height=14)
cowplot::plot_grid(
  cowplot::plot_grid(string_perm_res_dots,
                     network_egs,
                     ncol = 2,rel_widths = c(1,0.8),labels = c("A","B"),label_size = 32,label_x = c(0,-0.08)),
  GO_heat_bars_merge,
  nrow = 2,rel_heights = c(1,1.7),labels = c("","C"),label_size = 32,align = "v",axis = "tblr")
dev.off()

# Also save the enrichment results...
enrichment_export = rbindlist(climate_enrichment)
write.table(enrichment_export,
            'tables/TableSX_fig4_GO_enrichment_results.tsv',
