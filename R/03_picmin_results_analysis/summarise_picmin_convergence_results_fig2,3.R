...
# Save this
saveRDS(picmin_fdr_signif_dataset,
        paste0('outputs/',output_name,'_dataset_climate_signif.rds'))
...
climate_species_heat = ggplot(dataset_clim_counts,
                              aes(y=clim_F,
                                  x=species_F,
                                  fill=Freq/testedN))+
  geom_tile()+
...
# And a bar graph that shows strongest evidence of repeatability...
OG_fdr_bar_data = picmin_fdr[Orthogroup %in% OG_to_plot,.(min_FDR = min(picmin_fdr)),by = Orthogroup]
OG_fdr_bar_data = merge(OG_fdr_bar_data,unique(dataset_OG_counts[,.(Orthogroup,TAIR_genes)]),by = "Orthogroup")
OG_fdr_bar_data$TAIR_genes_F = factor(OG_fdr_bar_data$TAIR_genes,levels = levels(dataset_OG_counts$TAIR_genes_F))
OG_fdr_bars = ggplot(OG_fdr_bar_data,aes(y=TAIR_genes_F,x=-log10(min_FDR),fill=-log10(min_FDR)))+
  geom_bar(stat="identity",
           show.legend = F,fill = 'gold2',colour = 'black')+
  theme_minimal()+
  theme(axis.text.x = element_text(size=14,angle = 45,hjust = 1),
        ...
        legend.title = element_blank())+
  labs(x="Strongest\nevidence\nfor\nrepeatability (FDR)") +
  scale_x_continuous(breaks = c(-log10(0.5),
                                -log10(0.4),
                                -log10(0.3),
                                -log10(0.2),
                                -log10(0.1)),
                     labels = as.character(c(0.5,'','',0.2,0.1)))
OG_fdr_bars
...
OG_species_combined = cowplot::plot_grid(OG_species_heat + 
                                         ...
                                         OG_species_bars + 
                                           ...
                                           xlab("Sum"),
                                         OG_fdr_bars,
                                         ncol = 3,align="h",axis = "tblr",rel_widths = c(2,0.5,0.5))
OG_species_combined
...
# Repeat Heatmaps for Fig SX at FDR<0.3 -----------------------------------
# Figure 2 heatmaps -------------------------------------------------------
picmin_fdr_signif = picmin_fdr[picmin_fdr < 0.3,]
# Fetch all the datasets associated with each signif OG i.e. a pvalue < 0.1
picmin_fdr_signif_dataset <- rbindlist(lapply(1:nrow(picmin_fdr_signif),function(x){
...
# Plot climate vs species heatmap
climate_species_heat_fdr3 = ggplot(dataset_clim_counts,
                                   aes(y=clim_F,
                                       x=species_F,
                                       fill=Freq/testedN))+
  geom_tile()+
  ...
  theme(axis.title=element_blank(),
        ...
        legend.position = 'right')+
  labs(fill = expression(N/N[Tested])) +
  ggtitle("Contribution to RAOs (FDR < 0.3)\nby dataset and climate")
climate_species_heat_fdr3
...
to_plot_merge = data.table(merge(to_plot,species_tested_overlap_melt,all.x = T))
to_plot_merge[Var1 == Var2, testedN := 1]
dataset_dataset_heatmap_fdr3 = ggplot(to_plot_merge,aes(x=Var2_F,y=Var1_F,fill=value/testedN))+
  geom_tile()+
  scale_fill_viridis(option="A")+
  theme_minimal()+
  theme(axis.title=element_blank(),
        ...
        legend.text = element_text(size = 12),
        legend.title = element_text(size = 14))+
  ggtitle("Shared contribution to RAOs (FDR < 0.3)\n(same orthogroup, same climate)")+
  labs(fill=expression(N/N[Tested]))
dataset_dataset_heatmap_fdr3


# Save these as a supp fig
pdf('figs/FigureSX_fig3_heats_at_FDR03.pdf',width = 8,height = 14)
cowplot::plot_grid(
  climate_species_heat_fdr3,
  dataset_dataset_heatmap_fdr3,
  nrow = 2,align = 'v',axis = 'tblr',labels = 'AUTO',label_size = 32
)
dev.off()
...
# Make a supp table that lists all significant test results across all RAOs
contributing_species = picmin_fdr_signif_dataset[,.(contributing_species = paste(sort(.SD$dataset),collapse = ' - '),
                                                    contributing_species_N = nrow(.SD)),
                                                 by = .(Orthogroup,climate_var)]
contributing_species = merge(x = contributing_species,
                             y = picmin_fdr[,.(climate_var,
                                               Orthogroup,
                                               testN,
                                               config_est,
                                               picmin_p,
                                               picmin_fdr)],
                             by = c('Orthogroup','climate_var')) |>
  mutate(climate_var = stringr::str_to_title(gsub('_',' ',climate_var))) |>
  dplyr::rename(`Species with GEA p-val <0.1` = contributing_species,
                `Number of species with GEA p-val <0.1` = contributing_species_N,
                `Number of species in Orthogroup` = testN,
                `PicMin Config Estimate` = config_est,
                `PicMin p-val` = picmin_p,
                `PicMin FDR` = picmin_fdr,
                `Climate Var` = climate_var) 

# Re-order by FDR
contributing_species = contributing_species[order(`PicMin FDR`),]

# Tidy up
contributing_species$`PicMin FDR` = round(contributing_species$`PicMin FDR`,4)

# And add Athaliana gnees
OG_Athal_RAOs = OG_map_Athal[Orthogroup %in% contributing_species$Orthogroup,
                             .(`A thaliana genes` = paste(true_gene,collapse = '/')),
                             by = Orthogroup]
contributing_species = merge(OG_Athal_RAOs,contributing_species)

# Save this as a supp table
write.csv(contributing_species,
          'tables/TableSX_all_picmin_test_results_full.csv',
          quote = F,row.names = F)


# Formal test of phylogenetic signal in contributing species --------------
# Set up the tree
all_species_tree = ape::read.tree(text = all_species_nwk_BL)
# Split genus species
for(species in unique(OG_pvals$species)){
  all_species_tree$tip.label = gsub(gsub(' ','',species),
                                    species,
                                    all_species_tree$tip.label)
}
plot(all_species_tree)

# Calculate phylo signal for all RAOs
RAO_phylo = pbmclapply(1:nrow(picmin_fdr_signif),function(i){
  # Fetch the p-vals...
  rao_pvals = OG_pvals[climate_var == picmin_fdr_signif$climate_var[i] &
                         Orthogroup == picmin_fdr_signif$Orthogroup[i],]
  # Fetch vector
  pval_vector = -log10(rao_pvals$epval_final)
  names(pval_vector) = rao_pvals$species
  
  # Drop any tips that aren't included in this orthogroup
  to_drop = all_species_tree$tip.label[!all_species_tree$tip.label %in% names(pval_vector)]
  tmp_tree = drop.tip(all_species_tree,to_drop)
  pval_vector = pval_vector[tmp_tree$tip.label]
  
  # Test for phylo signal
  phylo_res = phytools::phylosig(tree = tmp_tree,
                                 x = pval_vector,test = T,method = 'K')
  out = data.table(Orthogroup = picmin_fdr_signif$Orthogroup[i],
                   climate_var = picmin_fdr_signif$climate_var[i],
                   K = phylo_res$K,
                   p = phylo_res$P)
},mc.cores = n_cores) |>
  rbindlist()

# Multiple comparison correction of these
RAO_phylo$fdr = p.adjust(RAO_phylo$p,method = 'fdr')
RAO_phylo[order(fdr),]
mean(RAO_phylo$K)

# Calculate phylo signal for all RAOs
iters = 100
picmin_fdr_random = picmin_fdr[picmin_fdr > 0.5,]
random_phylo = pbmclapply(1:iters,function(i){
  # Set a seed
  set.seed(i)
  
  # Pick some random OG and climate var
  randomOG = picmin_fdr_random[sample(1:nrow(picmin_fdr_random),nrow(picmin_fdr_signif)),.(climate_var,Orthogroup)]
  
  # Loop over these
  random_res = lapply(1:nrow(randomOG),function(i){
    # Fetch the p-vals...
    rao_pvals = OG_pvals[climate_var == randomOG$climate_var[i] &
                           Orthogroup == randomOG$Orthogroup[i],]
    # Fetch vector
    pval_vector = -log10(rao_pvals$epval_final)
    names(pval_vector) = rao_pvals$species
    ...
    phylo_res = phytools::phylosig(tree = tmp_tree,
                                   x = pval_vector,test = T,method = 'K')
    out = data.table(Orthogroup = randomOG$Orthogroup[i],
                     climate_var = randomOG$climate_var[i],
                     K = phylo_res$K,
                     p = phylo_res$P)
  }) |>
    rbindlist()
...
# Average K per iteration
random_clustering = random_phylo[,.(avg_K = mean(K),
                                    avg_pval = mean(-log10(p))),by = iter]
phylosig_res = ggplot(random_clustering,aes(x = avg_K)) +
  geom_histogram(bins = 10) +
  geom_vline(xintercept = mean(RAO_phylo$K),colour = 'red2',size = 2) +
  ggtitle('RAOs are not enriched for phylogenetic\nclustering of signatures of adaptation') +
  theme_minimal() +
  geom_vline(xintercept = mean(random_clustering$avg_K),
             colour = 'black',
             size = 1.5,
             linetype = 'dashed') +
  ...
  labs(x = 'Mean K-Clustering Statistic',y = 'Count')

# Plot a supp fig...
all_species_tree_fig = ggtree(all_species_tree) +
  geom_tiplab() +
  ggplot2::xlim(0, 1)
all_species_tree_fig

pdf('figs/FigureSX_phylosig_results.pdf',width = 10,height = 6)
cowplot::plot_grid(
  all_species_tree_fig,
  phylosig_res,
...
# Add a panel here for enrichment -----------------------------------------
# Randomised FDR results
random_FDRs = readRDS(paste0("outputs/",output_name,"_randomised_pvals_picmin_res_final_FDRs.rds"))

# Get counts of unique OG at FDR < 0.5 and raw
random_fdr_OGs = lapply(random_FDRs,'[[',2) |>
  rbindlist()
random_fdr_OGs = random_fdr_OGs[,.(sum_fdr_OG = length(unique(.SD$Orthogroup)),
                                   sum_fdr = nrow(.SD)),by = iteration]

# What are expecteds and max?
mean(random_fdr_OGs$sum_fdr_OG)
median(random_fdr_OGs$sum_fdr_OG)
max(random_fdr_OGs$sum_fdr_OG)

mean(random_fdr_OGs$sum_fdr)
median(random_fdr_OGs$sum_fdr)
max(random_fdr_OGs$sum_fdr)

# Plot as a pair of histograms
enrichment_figs = cowplot::plot_grid(
  
  ggplot(random_fdr_OGs,aes(x = sum_fdr)) +
    geom_histogram(bins = 20) +
    theme_minimal() +
    labs(x = 'Number of RAOs at FDR <0.5',y = 'Count') +
    ggtitle('Enrichment of ALL RAOs compared with 1000\nrandom permutations') +
    geom_vline(xintercept = nrow(picmin_fdr[picmin_fdr < 0.5,]),
               colour = 'red2',
               size = 1.5) +
    geom_vline(xintercept = median(random_fdr_OGs$sum_fdr),
               colour = 'black',
               size = 1.5,
               linetype = 'dashed') +
  ...
  ggplot(random_fdr_OGs,aes(x = sum_fdr_OG)) +
    geom_histogram(bins = 20) +
    theme_minimal() +
    labs(x = 'Number of RAOs at FDR <0.5',y = 'Count') +
    ggtitle('Enrichment of UNIQUE RAOs compared with 1000\nrandom permutations') +
    geom_vline(xintercept = length(unique(picmin_fdr[picmin_fdr < 0.5,Orthogroup])),
               colour = 'red2',
               size = 1.5) +
    geom_vline(xintercept = median(random_fdr_OGs$sum_fdr_OG),
               colour = 'black',
               size = 1.5,
               linetype = 'dashed') +
    theme(axis.text = element_text(size = 16),
  ...
  nrow = 2,axis = 'tblr',align = 'v',labels = 'AUTO',label_size = 32
)
enrichment_figs

# Combine all convergence result plots... ---------------------------------
# pdf("figs/Figure2_convergence_OG_results.pdf",width = 24,height = 11)
# cowplot::plot_grid(cowplot::plot_grid(enrichment_figs,
#                                       fdr_bars,
#                                       ncol = 1,rel_heights = c(1,2),labels = c('A','B'),label_size = 32),
#                    cowplot::plot_grid(climate_species_heat,dataset_dataset_heatmap,ncol=1,align = "v",axis = "tblr",labels = c("C","D"),label_size = 32),
#                    OG_species_combined,
#                    ncol = 3,labels = c("","","E"),label_size = 32,label_x = c(0,0,0.1),rel_widths = c(0.7,0.7,1.2))
# dev.off()

pdf("figs/Figure2_convergence_OG_results.pdf",width = 14,height = 8)
cowplot::plot_grid(cowplot::plot_grid(enrichment_figs,
                                      fdr_bars,
                                      ncol = 2,rel_widths = c(1,1),labels = c('','C'),label_size = 32))
dev.off()

pdf("figs/Figure3_convergence_OG_results.pdf",width = 16,height = 20)
cowplot::plot_grid(cowplot::plot_grid(climate_species_heat,
                                      dataset_dataset_heatmap,
                                      ncol=2,align = "h",axis = "tblr",labels = c("A","B"),label_size = 32),
                   OG_species_combined,
                   ncol = 1,
                   labels = c("","C"),
                   label_size = 32,
...
# Merge this with the lowest fdr
lowest_fdr = picmin_fdr_signif[,.(`Lowest FDR` = round(min(.SD$picmin_fdr),3)),by = Orthogroup]
signif_OG_summary = merge(signif_OG_summary,lowest_fdr)
# Prettify
signif_OG_summary = signif_OG_summary[order(-`Species Convergent N (FDR < 0.5)`,-`Total Convergent N (FDR < 0.5)`),]
