...
################################################
# Function library
# match_and_replace <- function(setx,sety){
#   
#   key_tab <- data.frame(setx=setx,
#                         newy=NA)
#   to_replace <- unlist(sapply(setx,function(x) grep(x,sety,value=T)))
#   for(i in 1:length(to_replace)){
#     key_tab[key_tab$setx == names(to_replace)[i],"newy"] <- to_replace[i]
#   }
#   return(key_tab)
# }
...
GEA_SNPs <- data.frame(fread(paste0(results_dir,"/",GEA_res[1]),fill=T))[,"snp_id"]
...
##########################################################################################
# Need an if statement to catch issues where the GFF does not match VCF chromosome identifiers
# test <- data.frame(snp_bed)
# test$chr <- gsub("Alyr_","",test$chr)

# if(length(gene_bed$chr[gene_bed$chr %in% snp_bed$chr]) == 0){
#   message("Warning, GFF/VCF chromosome identifiers do not match, attempting to resolve")
#   
#   gff_chr <- unique(gene_bed$chr)
#   snp_chr <- unique(test$chr)
#   
#   # Match each of these
#   gff_match <- unique(unlist(sapply(gff_chr,function(x) grep(x, snp_chr,value=T))))
#   snp_match <- unique(unlist(sapply(snp_chr,function(x) grep(x, gff_chr,value=T))))
#   
#   # Now edit the necessary one
#   if(length(gff_match) > 1){
#     message("Editing GFF chr to match VCF chr formatting")
#     replace_dict <- match_and_replace(gff_chr,snp_chr)
#     for(i in 1:nrow(replace_dict)){
#       gene_bed$chr <- gsub(replace_dict$setx[i],replace_dict$newy[i],gene_bed$chr)
#     }
#     
#   } else  if(length(snp_match) > 1){
#     message("Editing SNP chr to match GFF chr formatting")
#     replace_dict <- match_and_replace(snp_chr,gff_chr)
#     for(i in 1:nrow(replace_dict)){
#       snp_bed$chr <- gsub(replace_dict$setx[i],replace_dict$newy[i],snp_bed$chr)
#     }
#     
#   } else {
#     stop("Error: GFF and VCF chr identifiers cannot be matched")
#   }
# }
##########################################################################################
