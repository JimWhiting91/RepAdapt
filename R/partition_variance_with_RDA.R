...
# is.pool <- FALSE
# snp_downsample <- 50000
# res_dir <- "outputs/GEA_res/Amaranthus_tuberculatus_Wright_Individual///"
...
no_missing_rows <- which(rowSums(is.na(AF_mat)) == 0)
if(length(no_missing_rows) > snp_downsample){
  AF_mat_impute <- t(AF_mat[sort(sample(no_missing_rows,snp_downsample)),])
} else {
  AF_mat_impute <- apply(AF_mat[sort(sample(1:nrow(AF_mat),snp_downsample)),],1,impute_medians)
}
...
variance_partitioning_clean <- foreach(climate_var = climate_vars,.combine=rbind) %dopar% {
  ...
  # Variance partition...
  part1 <- varpart(AF_mat_trans,explanatory_merge[,c(climate_var)],explanatory_merge[,c("Long","Lat")])
  
  # Model SPEC
  spec_model <- rda(explanatory_merge[,c(climate_var)],explanatory_merge[,c("Long","Lat")])
  ...
  out <- data.frame(climate_var=climate_var,
                    ...
                    clim_clean=part1$part$indfract$Adj.R.squared[1],
                    clim_confound=part1$part$indfract$Adj.R.squared[2],
                    space_clean=part1$part$indfract$Adj.R.squared[3],
                    SPEC=RsquareAdj(spec_model)$r.squared,
                    SPEC_adj=RsquareAdj(spec_model)$adj.r.squared)
}

# Correct any adjusted R2 that are negative to zero...
to_adjust <- c("GSEC_adj","GSSC_adj","clim_clean","clim_confound","space_clean")
for(var in to_adjust){
  variance_partitioning_clean[,var][variance_partitioning_clean[,var] < 0] <- 0
}

# Calculate the abs and prop diff between unconfounded and confounded climate effects...
variance_partitioning_clean$clim_clean_abs <- variance_partitioning_clean$clim_clean - variance_partitioning_clean$clim_confound
variance_partitioning_clean$clim_confound_prop <- variance_partitioning_clean$clim_confound/rowSums(variance_partitioning_clean[,c("clim_clean","clim_confound")])

# # Test
# ggplot(variance_partitioning_clean,aes(SPEC_adj,clim_confound_prop))+geom_point()
...
#   climate_var_output[8,i] <- RsquareAdj(RDA_geog)$r.squared
#   climate_var_output[9,i] <- RsquareAdj(RDA_struct)$r.squared
...
saveRDS(variance_partitioning_clean,
        paste0(res_dir,"/variance_partitioning_RDA_results.rds"))
...
# test$GSSC <- test$`Geography_R-squared`
