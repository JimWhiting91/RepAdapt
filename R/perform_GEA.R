...
# Load these
lib <- c("parallel","data.table","sp","VGAM","argparse","raster","psych","pbmcapply","R.utils")
...
if(any(is.null(c(vcf_path,metadata_path)))){
  stop("ERROR: No VCF and/or METADATA provided")
}
if(is.null(n_cores)){
  n_cores <- 1
}
...
message(paste0("Metadata = ",metadata_path))
if(is.pool){
...
# Remove any individuals that do not have location data
sub_meta_species_code <- sub_meta_species_code[!(is.na(sub_meta_species_code$Lat) | is.na(sub_meta_species_code$Long)),]
...
# Report
message(paste0(species_code," contains ",length(unique(sub_meta_species_code$geo_pops))," geopops/pools"))
...
if(!is.pool){
  ...
  # Fetch all of the frequencies
  system(paste0("bcftools query -f '%CHROM\t%POS[\t%FREQ]\n' ",vcf_path," > ",workdir,"/pool_AFs.tsv"))
  system(paste0("sed -i 's/\t\\./\tNA/g' ",workdir,"/pool_AFs.tsv"))
  
  # Read in
  all_pool_AFs <- data.frame(fread(paste0(workdir,"/pool_AFs.tsv"),header=F))
  colnames(all_pool_AFs) <- c("chr","pos",vcf_inds)
  
  # Sort these and organise to match the above
  geo_pop_freqs <- mclapply(sub_meta_species_code$Sample_name,function(pool){
    ...
    geopop <- paste0(sub_meta_species_code[sub_meta_species_code$Sample_name == pool,"Lat"],"_",sub_meta_species_code[sub_meta_species_code$Sample_name == pool,"Long"])
...
for(x in 1:length(climate_vars)){
  ...
  cor_res <- pbmcapply::pbmclapply(1:length(winds),function(y){
    #cor_res <- lapply(1:length(winds),function(y){
  ...
  #out$snp_id  <- sapply(strsplit(out$snp_id,"_"),'[[',length(strsplit(out$snp_id,"_")[[1]][1]))
